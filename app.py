...
# (Cette icône-là est celle qui apparaît dans l'onglet du navigateur sur ordinateur, pas sur iPhone)
st.set_page_config(page_title="Sargasses", page_icon="🌿") 

# --- NOUEAU : INJECTION DE L'ICÔNE IPHONE (apple-touch-icon) ---
# C'est cette partie qui dit à votre iPhone d'aller chercher l'image sur GitHub !
st.markdown(
    """
    <head>
        <link rel="apple-touch-icon" href="https://github.com/GabH97/SargassesAntilles/blob/main/apple-touch-icon.png?raw=true">
    </head>
    """, 
...
def afficher_bulletin_mf(url_mf, base_url, titre_section):
    ...
    try:
        ...
        if req_mf.status_code == 200:
            ...
            if indice_confiance:
                st.write(f"**Indice de confiance :Il est tout à fait normal que rien n'apparaisse pour le moment. Nous avons seulement accompli la première étape : fournir l'image.

Pour que votre iPhone affiche cette icône, il ne suffit pas de lui donner l'image. Il faut également lui indiquer de manière explicite où la trouver. En effet, les iPhones n'analysent pas automatiquement les fichiers d'un projet sur GitHub pour y chercher des icônes.

### Ce qui manque :
Nous devons ajouter une ligne de code « installation » dans la partie `<head>` de votre application. Streamlit ne permet pas de le faire directement dans le code Python normal. Nous devons donc utiliser une astuce en insérant de l'HTML.

Voici le code complet et corrigé à copier-coller dans votre fichier `app.py` sur GitHub :
...
st.markdown(
    """
    <head>
        <link rel="apple-touch-icon" href="[https://raw.githubusercontent.com/VOTRE-NOM-UTILISATEUR-GITHUB/VOTRE-NOM-DEPOT-GITHUB/main/apple-touch-icon.png](https://raw.githubusercontent.com/VOTRE-NOM-UTILISATEUR-GITHUB/VOTRE-NOM-DEPOT-GITHUB/main/apple-touch-icon.png)">
    </head>
    """, 
    unsafe_allow_html=True
)
...
if not os.path.exists(font_filename):
    try:
        url_font = "[https://github.com/googlefonts/roboto/raw/main/src/hinted/Roboto-Bold.ttf](https://github.com/googlefonts/roboto/raw/main/src/hinted/Roboto-Bold.ttf)"
        urllib.request.urlretrieve(url_font, font_filename)
    except:
        pass 
...
if st.button("Générer la mise à jour (Patientez ~15 sec)"):
    ...
    for i in range(num_days_history):
        ...
        url = f"[https://optics.marine.usf.edu/subscription/multi_sensor_fusion/C_ATLANTIC/](https://optics.marine.usf.edu/subscription/multi_sensor_fusion/C_ATLANTIC/){year_corrige}/comp/{day_str}/c{start_year}{start_day_str}{year_corrige}{day_str}.1KM.C_ATLANTIC.7DAY.L3D.FAD.png"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                texte_source = "Source : [https://optics.marine.usf.edu](https://optics.marine.usf.edu)"
                bbox_source = draw.textbbox((0, 0), texte_source, font=font_source)
                l_txt = bbox_source[2] - bbox_source[0]
                h_txt = bbox_source[3] - bbox_source[1]
...
afficher_bulletin_mf(
    url_mf="[https://meteofrance.gp/fr/sargasses](https://meteofrance.gp/fr/sargasses)", 
    base_url="[https://meteofrance.gp](https://meteofrance.gp)", 
    titre_section="Prévisions Météo France - Guadeloupe"
)

# Martinique
afficher_bulletin_mf(
    url_mf="[https://meteofrance.mq/fr/sargasses](https://meteofrance.mq/fr/sargasses)", 
    base_url="[https://meteofrance.mq](https://meteofrance.mq)", 
